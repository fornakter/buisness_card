...
$fde37a429021a41b$exports = JSON.parse('{"name":"@parcel/fs","version":"2.7.0","description":"Blazing fast, zero configuration web application bundler","license":"MIT","publishConfig":{"access":"public"},"funding":{"type":"opencollective","url":"https://opencollective.com/parcel"},"repository":{"type":"git","url":"https://github.com/parcel-bundler/parcel.git"},"main":"lib/index.js","source":"src/index.js","types":"index.d.ts","engines":{"node":">= 12.0.0"},"targets":{"types":false,"main":{"includeNodeModules":{"@parcel/core":false,"@parcel/fs-search":false,"@parcel/types":false,"@parcel/utils":false,"@parcel/watcher":false,"@parcel/workers":false}},"browser":{"includeNodeModules":{"@parcel/core":false,"@parcel/fs-search":false,"@parcel/types":false,"@parcel/utils":false,"@parcel/watcher":false,"@parcel/workers":false}}},"scripts":{"build-ts":"mkdir -p lib && flow-to-ts src/types.js > lib/types.d.ts","check-ts":"tsc --noEmit index.d.ts"},"dependencies":{"@parcel/fs-search":"2.7.0","@parcel/types":"2.7.0","@parcel/utils":"2.7.0","@parcel/watcher":"^2.0.0","@parcel/workers":"2.7.0"},"devDependencies":{"graceful-fs":"^4.2.4","ncp":"^2.0.0","nullthrows":"^1.1.1","utility-types":"^3.10.0"},"peerDependencies":{"@parcel/core":"^2.7.0"},"browser":{"@parcel/fs":"./lib/browser.js","./src/NodeFS.js":"./src/NodeFS.browser.js"},"gitHead":"9e5d05586577e89991ccf90400f2c741dca11aa3"}');
...
function $d90b6e6bc50ca903$export$d51a93c758976388(fs, fileNames, dir, root) {
    let { root: pathRoot  } = (0, ($parcel$interopDefault($hunDZ$path))).parse(dir); // eslint-disable-next-line no-constant-condition
    while(true){
...
class $72ad4eda33ff61b0$export$3048eb7ec07c2c4e {
    ...
    _normalizePath(filePath, realpath = true) {
        filePath = (0, ($parcel$interopDefault($hunDZ$path))).resolve(this.cwd(), filePath); // get realpath by following symlinks
        if (realpath) {
    ...
    readdirSync(dir, opts) {
        ...
        for (let [filePath1, entry1] of this.files)if (filePath1.startsWith(dir) && filePath1.indexOf((0, ($parcel$interopDefault($hunDZ$path))).sep, dir.length) === -1) {
            let name1 = filePath1.slice(dir.length);
            if (opts?.withFileTypes) res.push(new $72ad4eda33ff61b0$var$Dirent(name1, entry1));
            else res.push(name1);
        }
        for (let [from] of this.symlinks)if (from.startsWith(dir) && from.indexOf((0, ($parcel$interopDefault($hunDZ$path))).sep, dir.length) === -1) {
            let name2 = from.slice(dir.length);
            if (opts?.withFileTypes) res.push(new $72ad4eda33ff61b0$var$Dirent(name2, {
    ...
    async rimraf(filePath) {
        ...
        if (this.dirs.has(filePath)) {
            ...
            for (let filePath1 of this.files.keys())if (filePath1.startsWith(dir)) {
                this.files.delete(filePath1);
                await this._sendWorkerEvent({
                    type: "unlink",
                    path: filePath1
                });
    ...
    async ncp(source, destination) {
        ...
        if (this.dirs.has(source)) {
            ...
            for (let [filePath, file] of this.files)if (filePath.startsWith(dir)) {
                let destName1 = (0, ($parcel$interopDefault($hunDZ$path))).join(destination, filePath.slice(dir.length));
                let exists = this.files.has(destName1);
                this.files.set(destName1, file);
                await this._sendWorkerEvent({
                    type: "writeFile",
                    path: destName1,
                    entry: file
...
const $31694e5356f611f3$var$pipeline = (0, $hunDZ$util.promisify)((0, ($parcel$interopDefault($hunDZ$stream))).pipeline); // Recursively copies a directory from the sourceFS to the destinationFS
